...
#revised 2011-03-14 #support for shino's sfs_load #cleaned up dead repos
#revised 2011-03-29 #4.4, work around for intel with same device ID as nvidia #RandSec
#revised 2011-05-23 #1.0 Slickpet (for spup.. actually a fork)
#revised 2011-10-01 #1.3.4 major rewrite, changed all config paths
#revised 2012-01-29 #1.7.3 added driver support for 3.1.10x kernels
...
VER=`cat $APPDIR/version`

#read config
. $CONFDIR/qpet.conf
. $CONFDIR/prefs.conf 2>/dev/null
. $CONFDIR/repos.conf 2>/dev/null
. $CONFDIR/sleeprc
. /etc/DISTRO_SPECS
. /etc/rc.d/PUPSTATE
...
debian)
   echo "you have dpup"
   # puppy5 repo code for now
   CONFFILE=$HOME/.packages/Packages-puppy-5-official
   BUTTON_IMG="/usr/share/doc/puppylogo48.png"
   ;;
slackware)
    CONFFILE=$HOME/.packages/Packages-puppy-slacko-official
     echo "you have spup"
   BUTTON_IMG="/usr/share/doc/puppylogo48.png"
   ;;
...
if [ ! -d $$HOME/.quickpet/Downloads ] ;then mkdir -p $HOME/.quickpet/Downloads
fi
KVER="`uname -r`"
SFSCONFIG=Sfs-puppy-spup-official-${KVER}
#check sfs loaded and adjust ### ~/.quickpet/Sfs-puppy-lucid-official
if [ -f $CONFDIR/$SFSCONFIG ];then
...
function WIKI(){
	...
	NEWSSITE="http://puppylinux.org/wikka/HomePage"   # was "http://www.diddywahdiddy.net/LupuNews"for lupu
	[[ "$DISTRO_BINARY_COMPAT" = "ubuntu" ]] && NEWSSITE="http://www.diddywahdiddy.net/LupuNews"
	[[ "$DISTRO_BINARY_COMPAT" = "slackware" ]] && NEWSSITE="http://puppylinux.org/wikka/SlackoNews"
...
function GETPET(){
	#set -x
	 ...
	 if  [[ `grep "#nvidialist" /tmp/conffile` = "" ]];then #only for nvidia drivers
		...
		if [ "$THEPET" = "Iron" ];then GRABPET=`grep -i "$THEPET" $CONFFILE|grep -v 'viron'| cut -d '|' -f8` #Iron fix bigpup #20110401 (not an April fools joke)
		 else GRABPET=`grep -i "$THEPET" $CONFFILE| cut -d '|' -f8`   #changed from grep -iw call #20101202 bigpup
		fi
		...
		for i in $GRABPET;do 
		 if [ "`echo $i|grep -i nvidia`" = "" ];then
		  echo $i >> /tmp/conffile
		 fi
		done
		 else NVIDIA_DLG="<hbox><text><label>Click for Nvidia $LOC_HELP</label></text><button><input file stock=\"gtk-help\"></input><action>$USEBROWSER $APPDIR/driverhelp.htm &</action></button></hbox> "
	 fi
   ...
   $NVIDIA_DLG
    <tree>
	 ...
	 if [ $PUPMODE = 3 ];then SAVESIZELEFT=`df -k | grep ' /initrd/pup_ro1$' | tr -s ' ' | cut -f4 -d ' '`
         ...
         elif [ $PUPMODE = 2 ];then SAVESIZELEFT=`df -k |  grep -w '/' | tr -s ' '| cut -f4 -d ' '`
         else SAVESIZELEFT=`df -k | grep ' /initrd/pup_rw$' | tr -s ' ' | cut -f4 -d ' '`
     fi
     PKGSIZEPLUS=`expr $PKGSIZE + 45000`
     if [[ $PKGSIZEPLUS -gt $SAVESIZELEFT && $SAVESIZELEFT -gt $PKGSIZE ]];then WARNINGINST
		...
		gtkdialog-splash -icon gtk-dialog-warning -timeout 5 -fontsize large -bg hotpink -close box -text "$LOC_CANTINSTALL $THEPET" &
		exit
	 fi
	 ...
	 kill -9 $YPID
	 (cd /tmp
	 download_file $URLREPO/$GRABPET
	 [ "$?" = "0" ] && petget +$DLDDIR/$GRABPET)
	 cd $HOME
	 sleep 1
	 ...
	 sleep 8 && exit	
	   ...
	   [ -x /pinstall.sh ] && rm -f /pinstall.sh 2>/dev/null #hmm this seems to be a bug in lupu
	     if [ $CHKBOX = "false" ];then 
           cp -f  $DLDDIR/$GRABPET $HOME/.quickpet/Downloads/$GRABPET
         fi
       sync #added for 4
	   sleep 8 && exit	  
	 fi
...
function RESET(){
	echo "SLEEP_SIZE=$SLEEP_SIZE" > $CONFDIR/sleeprc
	kill `ps | grep -w "quickpack" | awk '{print $1}'` 2>/dev/null
	quickpet &
}
 ...
 <vbox>
	...
	<checkbox>
     <label>$LOC_299</label>
	  ...
	  <action>if true sed -i 's/CHKBOX=[a-z]*[a-z]/CHKBOX=true/' $CONFDIR/prefs.conf</action>
	  <action>if false sed -i 's/CHKBOX=[a-z]*[a-z]/CHKBOX=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO1=[a-z]*[a-z]/RADIO1=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO1=[a-z]*[a-z]/RADIO1=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO2=[a-z]*[a-z]/RADIO2=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO2=[a-z]*[a-z]/RADIO2=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO3=[a-z]*[a-z]/RADIO3=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO3=[a-z]*[a-z]/RADIO3=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO4=[a-z]*[a-z]/RADIO4=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO4=[a-z]*[a-z]/RADIO4=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO5=[a-z]*[a-z]/RADIO5=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO5=[a-z]*[a-z]/RADIO5=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO6=[a-z]*[a-z]/RADIO6=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO6=[a-z]*[a-z]/RADIO6=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO7=[a-z]*[a-z]/RADIO7=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO7=[a-z]*[a-z]/RADIO7=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO8=[a-z]*[a-z]/RADIO8=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO8=[a-z]*[a-z]/RADIO8=false/' $CONFDIR/prefs.conf</action>
	...
	<action>if true sed -i 's/RADIO9=[a-z]*[a-z]/RADIO9=true/' $CONFDIR/prefs.conf</action>
	<action>if false sed -i 's/RADIO9=[a-z]*[a-z]/RADIO9=false/' $CONFDIR/prefs.conf</action>
...
<window title=\"$PROGNAME - About\"icon-name=\"gtk-about\" resizable=\"false\">
 <vbox>
	...
	<text><label>Russian - rodin.s</label></text>
...
#graphics test #updated 110523 thanks to James C
function graphics(){
	#set -x
...
Xor|mes) BUTTONIMG="$APPDIR/icons/xorg.png"
     ICONIMGTXT="Mesa"
     HELPBTN=""
     ...
     if [[ "`grep -iE "^mesa" $HOME/.packages/user-installed-packages 2>/dev/null`" != "" ]];then UPGRADEBTN=""
      UPGTXT="<hbox homogeneous=\"true\"><text><label>${ICONIMGTXT} $LOC_971b ${LOC_QUIT}!</label></text></hbox>"
     fi
;;
ATI|ati|amd) BUTTONIMG="$APPDIR/icons/ati.png"
     ICONIMGTXT="ATI"
     CURVIDEODRIVER="`grep -E -i " connected|card detect|primary dev" /var/log/Xorg.0.log|cut -d ':' -f1|rev|cut -d ' ' -f1|rev|cut -d '(' -f1|tr '[:upper:]' '[:lower:]'|head -n1"'`" #'geany#
     [ "$CURVIDEODRIVER" = "" ] && CURVIDEODRIVER="`grep 'Driver' /etc/X11/xorg.conf|grep -v 'Option'|cut -f 2 -d '"'`" #'geany #110816
     DRV_INST_ATI=`grep -i "fglrx" $HOME/.packages/user-installed-packages"`
     DRV_EXIST_ATI=`grep -i $DRVGRAB $CONFFILE|grep "$KERNEL_VER"`
     if [ "$DRV_EXIST_ATI" = "" ];then ACTION=`xmessage -bg hotpink "sorry no driver to suit your kernel available"`
      else
      GETDRIVER=`echo $DRV_EXIST_ATI|cut -d '|' -f2,3|tr '|' '-'`
      ACTION="echo $GETDRIVER > /tmp/thepet"
     fi
     HELPBTN="<button tooltip-text=\"$ICONIMGTXT opens $LOC_HELP in browser\"> 
     <label>${ICONIMGTXT} $LOC_HELP</label>    
     <input file stock=\"gtk-help\"></input>
     <action>$USEBROWSER $APPDIR/driverhelp.htm &</action>
    </button>"
     UPGRADEBTN="<button tooltip-text=\"$LOC_962 $DRVGRAB\"> 
     <label>${ICONIMGTXT}</label>    
     <input file>$BUTTONIMG</input>
...
;;
NVI|Luc|nvi) BUTTONIMG="$APPDIR/icons/nvidia.png"
     ICONIMGTXT="nVidia"
     CURVIDEODRIVER="`grep '#card0driver' /etc/X11/xorg.conf | cut -f 2 -d '"'`" #'geany
     DRV_INST_NVIDIA=`grep -i "nvidia" $HOME/.packages/user-installed-packages|grep "$KERNEL_VER" 2>/dev/null`
     DRV_EXIST_NVIDIA=`grep -i $DRVGRAB $CONFFILE|grep "$KERNEL_VER"`
     if [ "$DRV_EXIST_NVIDIA" = "" ];then ACTION=`xmessage -bg hotpink "sorry no driver to suit your kernel available"` 
      else
      GETDRIVER=`echo $DRV_EXIST_NVIDIA|cut -d '|' -f2,3|tr '|' '-'`
      ACTION="echo $GETDRIVER > /tmp/thepet"
     fi
     HELPBTN="<button tooltip-text=\"$ICONIMGTXT opens $LOC_HELP in browser\"> 
     <label>${ICONIMGTXT} $LOC_HELP</label>    
...
export CDRIVER=`cat /tmp/graphic_driver_current`
#test driver
...
<window title=\"Graphics\" resizable=\"false\">	
 <vbox>
  <text use-markup=\"true\"><label>\"Curent Driver:<b>$CDRIVER</b>\"</label></text>
...
function REPVID(){
	if [ -x /usr/sbin/report-video-glx ];then
	 report-video-glx
	 else
	 report-video
...
#sfs grab fn #Need to hardcode repo here for spup
function PROCESS(){
	SFSCONFIG="Sfs-puppy-spup-official-$KERNEL_VER" #Hmm.. hard coded for now 110523
	#SFSREPO="http://www.brainwavedesigncentral.net/micko01/stuff/puppy/spup/pet_packages-spup"
	...
	SFSCHOICEGRAB=`grep $SFSCHOICE $CONFDIR/$SFSCONFIG|cut -d '|' -f 8`
	SFSSIZE=`grep $SFSCHOICE $CONFDIR/$SFSCONFIG|cut -d '|' -f 6|sed 's/K$//'`
	...
	2|3)FREESIZEAVAIL=`df -k |  grep -w '/' | tr -s ' '| cut -f4 -d ' '` 
	...
	#echo "$SFSREPO/${SFSCHOICEGRAB}" > /tmp/grabthesfs
...
##update PPM #need to update for spup
PPMUPDATE(){
	rxvt -e wget -t3 -P /tmp http://distro.ibiblio.org/pub/linux/distributions/puppylinux/Packages-puppy-slacko-official 
	if [ -f /tmp/Packages-puppy-slacko-official ];then
    cp -f /tmp/Packages-puppy-slacko-official  $HOME/.packages/Packages-puppy-slacko-official
    rm -f /tmp/Packages-puppy-slacko-official
    rm -f /tmp/pet*
	xmessage -c -bg green "Success, now restart PetGet to activate update" 
	 else
...
cat $CONFDIR/$SFSCONFIG|cut -d '|' -f 2,10 > /tmp/sfstreelist
 
SFSTREELIST="cat /tmp/sfstreelist"
if [[ -f $CONFDIR/sfs_grab ]];then . $CONFDIR/sfs_grab
fi
export SFSTREELIST
...
DRIVERS='<hbox homogeneous="true">
 ...
 <text><label>'"$LOC_392vid"'</label></text>        
   <button relief="2" tooltip-text='\"$LOC_392ttvid\"'>     
     <input file>'"$APPDIR/icons/display.png"'</input>
 ...
 <text><label>'"$VIDDRIVE"'</label></text>        
   <button relief="2">     
     <input file>'"$APPDIR/icons/xorg.png"'</input>
...
#news
 ...
 lupu|luma|luci|lupq)  #$HOME/.quickpet/icons/lupu.png
  ...
  <button>  
    ...
    <input file>$$APPDIR/icons/lupu.png</input>
    <action>exit:wiki</action>
   </button>
   </vbox>"
   ;;
 qrky|qret)  
 ...
 <menubar>
    <menu>
         ...
         <menuitem icon=\"gtk-directory\" tooltip-text=\"$LOC_353\">
           <label>$LOC_351</label>
           <action>rox $HOME/.quickpet/Downloads</action>
         </menuitem>
      ...
      <menu>
         ...
         <menuitem tooltip-text=\"restart petget\">
           <label>Update Puppy Package Manager</label>
...
I=$IFS; IFS="" 
for STATEMENTS in  $(gtkdialog -p quickpack); do 
	eval $STATEMENTS
done
IFS=$I

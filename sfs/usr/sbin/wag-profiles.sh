  ...
  <hbox>
   <button>
	...
	(
		...
		if dhcpcd -d -I '' "$INTERFACE" 2>&1 | tee /tmp/dhcpcd.log
		then
			HAS_ERROR=0
			grep -q 'Error' /tmp/dhcpcd.log && HAS_ERROR=1
		else
			HAS_ERROR=1
		fi
...
showProfilesWindow()
{
	...
	case "$INTMODULE" in 
	 ...
	 ar9170usb|carl9170*|at76c50x-usb|libertas_cs|libertas_sdio|libertas_tf_usb|mwl8k|usb8xxx) CARD_WPA_DRV="wext" ;; #v430 #added carl9170* Feb 2012
